...
function handleTouchEnd(touchStart: number, touchEnd: number, mapContext: MapData) {
  if (
  ...
  if (mapContext.current.area === buildingNames.CAMPUS) {
    return (
      ...
      >
        ...
        <button onClick={() => handleHighlightLeo11(mapContext)}>Where is Leo 11?</button>
      </div>
    );
